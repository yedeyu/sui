...
fn execute_inner(
    ...
    match bytecode {
        ...
        Bytecode::PackVariant(vidx) => {
            let handle = verifier.resolver.variant_handle_at(*vidx)?;
        ...
        Bytecode::PackVariantGeneric(vidx) => {
            let handle = verifier.resolver.variant_instantiation_handle_at(*vidx)?;
            let enum_def = verifier.resolver.enum_instantiation_at(handle.enum_def)?;
            let variant_def = verifier
                .resolver
