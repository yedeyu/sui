...
async fn test_transaction_locks_are_exclusive() {
    ...
    Scenario::iterate(|mut s| async move {
        ...
        let lock = authority.execution_lock_for_signing(0).await.unwrap();
        ...
        s.cache
            .acquire_transaction_locks(&s.epoch_store, &lock, &[new1, new2], tx2.clone())
            .await
            .unwrap_err();

        // new3 is lockable, but new2 is not, so this should fail
        s.cache
            .acquire_transaction_locks(&s.epoch_store, &lock, &[new3, new2], tx2.clone())
            .await
            .unwrap_err();

        // new3 is unlocked
        s.cache
            .acquire_transaction_locks(&s.epoch_store, &lock, &[new3], tx2)
            .await
...
async fn test_acquire_transaction_locks_revert() {
    ...
    Scenario::iterate(|mut s| async move {
        ...
        // fails because we are referring to an old object
        s.cache
            .acquire_transaction_locks(&s.epoch_store, &lock, &[new1, old2], tx)
            .await
            .unwrap_err();
    ...
    let t1 = {
        ...
        tokio::task::spawn(async move {
            ...
            for (tx1, _, a_ref, b_ref) in txns {
                results.push(
                    cache
                        .acquire_transaction_locks(&epoch_store, &lock, &[a_ref, b_ref], tx1)
                        .await,
                );
                barrier.wait().await;
            }
    ...
    let t2 = {
        ...
        tokio::task::spawn(async move {
            ...
            for (_, tx2, a_ref, b_ref) in txns {
                results.push(
                    cache
                        .acquire_transaction_locks(&epoch_store, &lock, &[a_ref, b_ref], tx2)
                        .await,
                );
                barrier.wait().await;
            }
